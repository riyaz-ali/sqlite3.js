...
import { isArray, isArrayBuffer, isBoolean, isNumber , isNull, isString } from 'lodash';
...
const bind = function(stmt, pos, val) {
  if(isString(val)) {
    ...
    let fn = (val === (val | 0)) || isBoolean(val)? sqlite3.sqlite3_bind_int : sqlite3.sqlite3_bind_double;
...
export default class Statement {
  ...
  bindParams(params) {
    if(isArray(params)) {
      each(params, (param, idx) => bind(this.handle, idx+1, param)) // params are 1-indexed
    ...
    for (let pos = 0; pos < n; pos += 1) {
      switch (sqlite3.sqlite3_column_type(this.handle, pos)) {
        ...
        case 4: /* SQLITE_BLOB */ {
          ...
          buf.set(heap);
          results.push(transfer(buf, [buf.buffer])); // transfer the buffer to ensure no further copying
        } break;
        default: /* SQLITE_NULL */ {
          results.push(null);
        }
